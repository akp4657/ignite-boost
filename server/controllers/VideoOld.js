...
const makeVideo = async (req, res) => {
  ...
  for (let i = 0; i < values.length - 2; i++) {
    ...
    videoPromise.catch((err) => {
      console.error(err);
      if (err.code === 11000) {
        return res.status(400).json({ error: 'ERROR | Video already exists.' });
      }

      return res.status(500).json({ error: 'ERROR | Unable to add videos, please try again later.' });
    });
  ...
  Promise.all(promiseArray).then(() => res.status(200).json({ redirect: '/' }));
};
...
const getVideos = (req, res) => {
  return Video.VideoModel.findByOwner(req.session.account._id, (err, docs) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: 'ERROR | Unable to get videos, please try again later.' });
    }
...
const getAllVideos = (req, res) => {
  return Video.VideoModel.findAll((err, docs) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: 'ERROR | Unable to get all videos, please try again later.' });
    }
...
const deleteEntry = (req, res) => {
  Video.VideoModel.deleteItem(req.body.uid, (err, docs) => {
    if (err) {
      console.log(err);
      return res.status(500).json({ error: 'ERROR | Unable to delete video, please try again later.' });
    }
...
const searchVideos = (req, res) => {
 ...
 if(char1 && char2) {
  ...
  return Video.VideoModel.find(params, (err, docs) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: 'ERROR | Unable to search videos, please try again later.' });
    }
